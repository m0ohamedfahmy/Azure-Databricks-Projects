...
from pyspark.sql.functions import current_timestamp ,col,when,count,desc, rank, asc,sum
...
# race_year_list=[]
# for year in race_results_list:
#     race_year_list.append(year.race_year)
race_results_df = spark.read.option("inferSchema", True).format("delta").load(f"{presentation_folder_path}/race_results") \
    .filter(f"file_date = '{v_file_date}'")
